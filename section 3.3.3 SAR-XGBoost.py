...
if __name__ == "__main__":
    ...
    df_sdm = pd.read_excel("/Users/yangjing/Desktop/dissertation/SAR.xlsx")
    ...
    for day in range(22,29):
        for i in range(0,24):
            ...
            df_temp = df_sdm[df_sdm["日期"]==day]
            df_temp = df_temp[df_temp["小时"]==i]
            dic_micro["spatial"].append(df_temp["spatial"].mean())
    x = pd.DataFrame(dic_micro)#complete 
    ...
    y = y.values
    y = y.astype(np.float)
    # y = pd.DataFrame(np.random.randint(0,6,size=(23, 1)))
    print(y)
    print("\n"*3)
    print("---------------------------------Summary of XGBoost model parameters ---------------------------------\n")
    ...
    # feature = np.array([0.87751926,0.05,0.05781318,0.05,0.05,0.05,
    #                                      0.01466756,0.05,0.05,0.05,0.05,0.05,])
    print(xgb.feature_importances_)
    # plt.barh(name, feature)
    plt.barh(name, xgb.feature_importances_)
    plt.tight_layout()
    plt.savefig("/Users/yangjing/Desktop/dissertation/SAR-XGBoost", dpi=600)
    plt.show()
